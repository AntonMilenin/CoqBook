...
Inductive pal {X:Type} :list X -> Type :=
  |one : forall n:X, pal (n::[])
  |zero : pal []
  |many : forall (n:X) (l:list X) ,  pal (rev l) -> pal (n::(snoc l n)).

Theorem app1 : forall (X:Type) (l: list X), l++[]=l.
Proof.
  intros.
  induction l as [|h l']. 
  Case "l = []". simpl. 
...
Theorem app2 : forall (X:Type) (l1 l2 l3: list X),  (l1 ++ l2)++ l3=l1 ++ (l2++l3).
Proof.
  intros.
...
Theorem rev3 : forall (X:Type) (l1 l2: list X),  (rev l1 ++ rev l2)=rev(l2 ++ l1).
Proof.
  intros.
  induction l2 as [|h l']. 
  Case "l = []". simpl.
...
Fixpoint wlt {X:Type} (l:list X):list X:=
  match l with 
  |[]=>[]
  |[h]=>[]
  |h::l'=>h::(wlt l')
...
Theorem rev2 :forall (X:Type) (l:list X), [] = rev l -> l = [].
Proof.
  intros.
  induction l as [|h l']. 
  Case "l = []". 
...
Proof.
  ...
  Case "l = h :: l'".
    ...
    replace (l') with (snoc (wlt l') h).
    apply many.

    remember (rev l' ) as re.
    induction re as [|h' l''].
    rewrite rev2 with (l:=l').
    apply one.
    apply Heqre.
    apply IHl''.
    destruct l'.

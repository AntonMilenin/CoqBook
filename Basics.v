...
Выражение сократится только во втором случае по первой ветке match
в определении сложения:
Fixpoint plus (n : nat) (m : nat) : nat :=
  match n with
    | O => m
    | S n' => S (plus n' m)
  end.
В первом случае ни одно правило не может быть применено. 
(Первоее не подходит, а второе не может оперировать с неизвестной переменной.)
...
destruct просто разбирает различные варианты определения переменной.
induction тоже разбирает различные варианты определения, но при этом 
предпологает, что если индуктивная переменная определяется через другую переменную(ые)
того же типа, то для этих переменных утверждение уже доказано.  
...
(** Theorem: Addition is commutative.
    ...
    Proof: проведём индукцию по n.
Пусть n = 0. Тогда утверждени истино по определению сложения и теореме n + 0 = n.

Пусть n = S n' и n' + m = m + n'. 
Покажем, что S n' + m = m + S n'.
По теореме plus_n_Sm : forall n m : nat,  S (n + m) = n + (S m)
это утверждение следует из S (n' + m) = S (m + n'), что является 
следствием предположения индукчции.
[]
*)
...
(** Write an informal proof of the following theorem, using the
    ...
    Proof: проведём индукцию по n.
Пусть n = 0. Тогда утверждени истино по определению beq_nat.

Пусть n = S n' и true = beq_nat n' n'. 
Покажем, что true = beq_nat S n' S n'.
Это утверждение следует из true = beq_nat n' n' по определению beq_nat.
Что является предположением индукции. 
[]
 *)
...
Proof. 
 ...
 rewrite -> plus_swap.
 rewrite -> IHn'. reflexivity.  Qed.
(*Мне кажется, что тут надо придумать доказательсво без индукции, 
но сейчас ничего кроме этого прпидумать не могу*)

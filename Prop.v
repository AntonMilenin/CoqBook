...
(* Infinit we may add additions of beautiful 0 in any proof.  *)
(** [] *)
...
Theorem mul2l: forall n, 2*n = n + n.
Proof.
  intros n.
  simpl.
  rewrite plus_0_r_firsttry.
...
Definition b_times2': forall n, beautiful n -> beautiful (2*n) := 
  fun n => fun H : beautiful n =>
    (b_sum n (n + 0) H (eq_ind_r (fun n' => beautiful n') H (plus_0_r_firsttry n))).
Check b_plus3'.
...
Definition double_even' :forall n, ev (double n):=
  fun n=> nat_ind (fun n'=>ev (double n')) ev_0 (fun n' IHn'=> ev_SS (double n') IHn') n.
...
(** What happens if we try to [destruct] on [n] instead of [E]? *)
  (*It will be inpossible to use E because arguments of E and goal will be different everytime.*)
(** [] *)
...
Proof.
  ...
  apply H.
  apply H0.
  replace (n + m + (n + p)) with (double(n) + (m + p)) in H1.
  apply ev_ev__ev with (n:=double(n)).
  apply H1.
  apply double_even.
  replace (double n) with ( n + n).
  rewrite helper_g_times2.
  rewrite helper_g_times2.
  rewrite plus_assoc.
  simpl.
...
(*tree_ind :
       forall (X : Type) (P : tree X -> Prop),
          ...
          (forall t1 : tree X, P t1 -> forall t2 : tree X, P t2 -> P (node X t1 2)) ->
          forall t : tree X, P t*)
...
Definition plus_assoc_pr: nat -> nat -> nat -> Prop:=
  fun n m p => n + (m + p) = (n + m) + p.  

Theorem plus_assoc'' : forall n m p : nat, plus_assoc_pr n m p.   
Proof.
  intros n m p. 
  induction n as [| n'].
  Case "n = O". reflexivity.
  Case "n = S n'". 
    unfold plus_assoc_pr.
    simpl. rewrite -> IHn'. reflexivity.  Qed.

Definition plus_comm_pr: nat -> nat -> Prop:=
  fun n m => n + m = m + n.  
...
Inductive pal {X:Type} :list X -> Type :=
  |one : forall n:X, pal (n::[])
  |zero : pal []
  |many : forall (n:X) (l:list X) ,  pal l -> pal (n::(snoc l n)).

Theorem pal1 : forall (X:Type) (l: list X), pal (l ++ rev l).
Proof.
  intros.
  induction l as [|h l']. 
  Case "l = []". simpl. 
...
Theorem revTl :forall (X:Type) h (l:list X), h :: l = rev (h :: l) -> tl(rev (h :: l))=l.
Proof.
  intros.
  destruct l as [|h' l']. 
  Case "l = []". 
...
Theorem tl1 :forall (X:Type) h h1 (l:list X), tl ((h1::l)++h) = tl (h1::l)++h.
Proof.
  intros.
  induction l as [|h' l']. 
  Case "l = []". 
...
(*
Theorem pal2 :forall (X:Type) (l:list X), l = rev l -> pal l.
...
Proof.
  ...
  Case "l = h :: l'".
    inversion H.
    remember (rev l ) as re.
    induction re as [|h l'] .
    unfold  Heqre.
    
    replace (l') with ((tl (rev (h::l')))).
    simpl.
    rewrite snoc_append.
    remember (rev l' ) as re.
    induction re as [|h' l''] .
    ...
    apply many.
    apply IHl'.
    rewrite <- revTl with (h:=h).
    assert (H2:tl(rev l')=l').
induction l'.
reflexivity.
    rewrite H1.
    inversion H.
...
Inductive subsequence :list nat -> list nat -> Type :=
  |sub1 : forall (l:list nat) , subsequence [] l
  |sub2 : forall (n: nat) (l1 l2:list nat) , subsequence l1 l2->subsequence l1 (n::l2)
  |sub3 : forall (n: nat) (l1 l2:list nat) , subsequence l1 l2->subsequence (n::l1) (n::l2).

Theorem reflSub :forall (l:list nat), subsequence l l.
Proof.
  intros.
  induction l as [|h l']. 
  Case "l = []". 
...
Theorem tSub :forall (l1 l2 l3:list nat), subsequence l1 l2 -> subsequence l1 (l2++l3).
Proof.
  intros.
  induction H.
    apply sub1.
...
(*
Theorem tSub1 :forall (l1 l2 l3:list nat), subsequence l1 l2 -> subsequence l2 l3 -> subsequence l1 l3.
Proof.
  intros.
  induction H.
    apply sub1.
...
(** Suppose we make the following inductive definition:
   ...
   foo_ind
        : forall (X Y : Set) (P : foo X Y -> Prop),   
          (forall x : X, P (foo X Y x)) ->
          (forall y : Y, P (foo X Y y)) ->
          (forall f: foo X Y, P f -> P (foo X Y f)) ->
...
(* 
    - [R 6 [3,2,1,0]] unprovable. We can apply only c3. But it will not let to use c2. *)
(** [] *)
